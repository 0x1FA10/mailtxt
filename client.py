...
class SMSMailer:
    ...
    def send_sms(self, phone_number, carrier, message_body):
        ...
        try:
            conn.connect()
            for cmd in ["AUTH LOGIN",
                    base64.b64encode(self.smtp_settings.username.encode()).decode(),
                    base64.b64encode(self.smtp_settings.password.encode()).decode()]:
                resp = conn.send_command(cmd)
                if not (resp.startswith("334") or resp.startswith("235")): raise RuntimeError(f"Auth failed: {resp}")
            for cmd, err in [(f"MAIL FROM:<{self.smtp_settings.username}>", "MAIL FROM rejected"),
            ...
            msg = (f"From: {self.smtp_settings.username}\r\nTo: {to_address}\r\nSubject: [ALERT]\r\n"
               "Content-Type: text/plain; charset=utf-8\r\n\r\n" + message_body + "\r\n.\r\n")
            conn.sock.sendall(msg.encode())
            if not conn._recv().startswith("250"): raise RuntimeError("Message send failed")
